...
function Calendar({
  ...
  const CustomCaption = (captionProps: CaptionProps) => {
    ...
    // تحويل الشهور إلى قائمة للاختيار
    ...
    // تحويل السنوات إلى قائمة للاختيار
    const years = React.useMemo(() => {
      const years = [];
      const fromYear = fromMonth?.getFullYear() || new Date().getFullYear() - 10;
      const toYear = toMonth?.getFullYear() || new Date().getFullYear() + 10;
    ...
    return (
      <div className="flex justify-center gap-1 rtl:flex-row-reverse">
        ...
        >
          <SelectTrigger className="h-7 w-[60px] text-xs font-normal">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="max-h-[200px] overflow-y-auto">
            {years.map((year) => (
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        caption_dropdowns: "flex justify-center gap-1 rtl:flex-row-reverse",
